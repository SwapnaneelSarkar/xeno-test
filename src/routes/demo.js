...
// Add demo data for test7@gmail.co account
router.post('/add-demo-data', async (req, res) => {
  try {
    ...
    const tenant = await prisma.tenant.findUnique({
      where: { email: 'test7@gmail.co' }
    });

    if (!tenant) {
      return res.status(404).json({
        success: false,
        message: 'Tenant not found. Please register the account first.'
      });
    }
    ...
    res.status(500).json({
      success: false,
      message: 'Error adding demo data',
      error: error.message
    });
  }
});
...
router.get('/demo-summary', async (req, res) => {
  try {
    ...
    const totalRevenue = tenant.orders.reduce((sum, order) => sum + Number(order.totalPrice), 0);
    const avgOrderValue = tenant.orders.length > 0 ? totalRevenue / tenant.orders.length : 0;
    ...
    console.error('❌ Error getting demo summary:', error);
    res.status(500).json({
      success: false,
      message: 'Error getting demo summary',
      error: error.message
    });
  }
});
